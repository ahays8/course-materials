...
func main() {
	...
	//TODONE - Try to find these (you may or may not based on your password lists)
	...
	//Note: the list has 303872 passwords

	//TODONE Grab the file to use from the command line instead; look at previous lab (e.g., #3 ) for examples of grabbing info from command line

	if len(os.Args) != 2 {
		log.Fatalln("Usage: main <passwordlist>")
	} else {
		var file = os.Args[1]
		hscan.GuessSingle(md5hash, file)
		hscan.GuessSingle(sha256hash, file)

		_ = hscan.GuessSingle(drmike1, file)
		_ = hscan.GuessSingle(drmike2, file)

		_,_=hscan.GenHashMaps(file)
